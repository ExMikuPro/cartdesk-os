...
QFlash littlefs。支持 boolean、32 位 integer、number、string；key 最长 64 B，string
...
`random.integer(min,max)` 含两端并使用 rejection sampling，`random.number()` 返回
`[0,1)`，`random.bytes(length)` 返回最多 4096 B 的 binary string。三者仅调用统一
`RNG_GetU32/RNG_Fill` 驱动，不向 Lua 暴露 HAL handle。
...
固件 Lua 使用 32 位 integer；大于 `0x7FFFFFFF` 的 CRC 以能精确表示的 Lua number
返回，数值仍保持非负 32 位范围。
