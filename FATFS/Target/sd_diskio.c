...
static osMessageQueueId_t SDQueueID
    __attribute__((section(".sdmmc_ram_data"), aligned(32))) = NULL;
#endif
...
static int SD_IsDmaAccessibleBuffer(const void *buffer, uint32_t byte_count)
{
  const uintptr_t start = (uintptr_t)buffer;
  const uintptr_t end = start + (uintptr_t)byte_count;
  const uintptr_t dtcm_start = 0x20000000u;
  const uintptr_t dtcm_end = 0x20020000u;

  if ((byte_count == 0u) || (end < start))
  {
    return 0;
  }

  /* STM32H743 SDMMC1 IDMA cannot access the Cortex-M7 DTCMRAM window. */
  return (end <= dtcm_start) || (start >= dtcm_end);
}
